    ...
    def update(self):
        ...
        with torch.no_grad():
            if(self.exp_bad_buffer.roll_n>=self.start_bad and self.n_step>=self.warm_start_steps):
                ...
                # 应用自适应偏差校正
                current_violation_rate = np.mean(self.return_cost[-100:]) if len(self.return_cost) >= 100 else 0.0
                delta = self.bias_corrector.compute_adaptive_delta(
                    training_progress, current_violation_rate
                )

                costs_clfs = self.bias_corrector.apply_bias_correction(raw_costs, uncertainty)

                # 更新统计信息
                if hasattr(self, 'return_cost') and len(self.return_cost) > 0:
                    true_costs = torch.tensor([self.return_cost[-len(costs_clfs):]], device=self.device).float()
                    if len(true_costs) == len(costs_clfs):
                        self.bias_corrector.update_statistics(costs_clfs, true_costs, current_violation_rate)
