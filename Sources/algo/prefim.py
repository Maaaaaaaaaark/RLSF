...
class PREFIM(PPO_lag):
    def __init__(self,env_name,exp_good_buffer, exp_bad_buffer, tmp_query_buffer, state_shape, action_shape, device, seed, gamma,cost_gamma,
        ...
        self.segment_corrector = SegmentLevelBiasCorrector(
            segment_length=segment_length if segment_length else max_episode_length,
    ...
    def step(self, env, state, ep_len, n_step):
        ...
        for idx in range(self.num_envs):
            ...
            if done[idx]:
                ...
                self.tmp_buffer[idx] = []
                self.return_cost.append(np.sum(self.tmp_return_cost[idx]) / ep_len[idx])
    ...
    def update(self):
        ...
        if(self.aug_state):
            states, actions, env_rewards, _, true_costs_buffer, dones, log_pis, next_states, aug_states = self.buffer.get()

        else:
            states, actions, env_rewards, _, true_costs_buffer, dones, log_pis, next_states = self.buffer.get()
        
        rewards = env_rewards
        with torch.no_grad():
            if(self.exp_bad_buffer.roll_n>=self.start_bad and self.n_step>=self.warm_start_steps):
                # 使用改进的不确定性感知成本估计
                input_states = aug_states if self.aug_state else states

                # 计算集成预测和不确定性
                mean_probs, uncertainty, individual_probs = self.uncertainty_estimator.compute_ensemble_predictions(
                    self.clfs, input_states, actions
                )

                # 计算不确定性感知的成本
                training_progress = min(1.0, self.n_step / 100000)  # 假设总训练步数为100k
                raw_costs, confidence = self.uncertainty_estimator.compute_uncertainty_aware_costs(
                    mean_probs, uncertainty, self.class_prob
                )

                # 1. 获取真实的平均违规率 (Bug A 已修复)
                current_violation_rate = np.mean(self.return_cost[-100:]) if len(self.return_cost) > 0 else 0.0

                # 2. (修正) *首先*更新偏差估计。
                #    必须使用 "raw_costs" (分类器原始预测) 来和 "true_costs_buffer" 比较。
                #    这是修复数据污染 (Bug C) 的关键。
                self.bias_corrector.update_statistics(
                    raw_costs, 
                    true_costs_buffer, 
                    current_violation_rate
                )

                # 3. (修正) *然后*在 bias_estimate 更新*之后*，再计算新的 delta。
                #    compute_adaptive_delta 会在内部更新 self.delta
                _ = self.bias_corrector.compute_adaptive_delta(
                    training_progress, 
                    current_violation_rate
                )
                # [--- 在这里添加你的新日志 ---]
                delta_val = self.bias_corrector.delta
                bias_val = self.bias_corrector.bias_estimate
                print(f"[BiasCorrector] update() running: delta={float(delta_val):.6f}, bias_estimate={float(bias_val):.6f}")
                
                # (修正) 在这里添加 wandb 日志
                if self.wandb_log:
                    wandb.log({
                        'improved/delta': float(delta_val),
                        'improved/bias_estimate': float(bias_val)
                    })

                # 4. (修正) *最后*，使用刚更新的 self.delta 来应用校正
                costs_clfs = self.bias_corrector.apply_bias_correction(
                    raw_costs, 
                    uncertainty
                )

                self.uncertainty_estimator.update_uncertainty_statistics(uncertainty, mean_probs)
                ...
                if self.n_step % 100 == 0:
                    ...
                    high_conf_ratio = (confidence > self.uncertainty_estimator.confidence_threshold).float().mean().item()

                    # 计算集成分歧度（验证集成有效性）
                    ensemble_disagreement = torch.std(individual_probs, dim=0).mean().item()

                    # 统计明确危险 vs 不确定区域
                    high_cost_high_conf = ((raw_costs > 0.5) & (confidence > 0.8)).float().mean().item()
                    high_cost_low_conf = ((raw_costs > 0.5) & (confidence <= 0.8)).float().mean().item()

                    print(f"[Uncertainty] unc={unc_mean:.4f}±{unc_std:.4f}, conf={conf_mean:.4f}, high_conf%={high_conf_ratio*100:.1f}%, "
                          f"disagreement={ensemble_disagreement:.4f}, danger_certain={high_cost_high_conf*100:.1f}%, danger_uncertain={high_cost_low_conf*100:.1f}%")
