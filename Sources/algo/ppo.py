...
def calculate_gae_cost(values, rewards, dones, next_values, gamma, lambd):
    ...
    # 统一形状为 (T, 1) 的列向量，避免广播造成的维度不一致
    def ensure_col(t: torch.Tensor) -> torch.Tensor:
        if t.dim() == 0:
            return t.view(1, 1)
        if t.dim() == 1:
    ...
    values = ensure_col(values)
    rewards = ensure_col(rewards)
    dones = ensure_col(dones)
    next_values = ensure_col(next_values)

    # Calculate TD errors. 形状统一为 (T, 1)
    deltas = rewards + gamma * next_values * (1 - dones) - values
